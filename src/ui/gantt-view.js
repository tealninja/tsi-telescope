/* ============================================================
   Gantt view  ·  phase timeline + drag-to-shift / drag-to-reorder,
   conflict rings, pin glyphs, plus the two collapsible companion
   panels (Portfolio Resources + Portfolio Cashflow) that share the
   Gantt's range zoom.

   Module-local state:
     _ganttRange         visible month-index range (shared with companions)
     _ganttDragState     active drag session (null when idle)
     _ganttResources*    mode + collapsed state for the resources panel
     _ganttCashflow*     mode + collapsed state for the cashflow panel
   ============================================================ */
...
import {
  state, getRole, getPhase, getLocation, getProject, horizonMonths, saveState
} from '../state.js';
import {
  effectivePhaseDurations, projectStackOrder
} from '../compute/demand.js';
import { computePortfolioCashflow } from '../compute/cashflow.js';
import {
  computeProjectConflicts, computeTotalSeries
